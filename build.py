...
hbb_name = '4desk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: 4desk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: 4desk <tarek.buhdeima@gmail.com>
Homepage: https://github.com/Tarek-Bohdima/4desk
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/4desk')
    system2('mkdir -p tmpdeb/etc/4desk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/4desk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/4desk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/4desk/')
    system2(
        'cp ../res/4desk.service tmpdeb/usr/share/4desk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/4desk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/4desk.svg')
    system2(
        'cp ../res/4desk.desktop tmpdeb/usr/share/applications/4desk.desktop')
    system2(
        'cp ../res/4desk-link.desktop tmpdeb/usr/share/applications/4desk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/4desk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/4desk/')
    system2(
        'cp ../res/pam.d/4desk.debian tmpdeb/etc/pam.d/4desk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/4desk/files/polkit && chmod a+x tmpdeb/usr/share/4desk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb 4desk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('4desk.deb', '../4desk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/4desk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/4desk/')
    system2(
        'cp ../res/4desk.service tmpdeb/usr/share/4desk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/4desk.png')
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/4desk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"4desk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon 4desk.app 200 190 --hide-extension 4desk.app 4desk.dmg ./build/macos/Build/Products/Release/4desk.app")
    os.rename("4desk.dmg", f"../4desk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/4desk.exe')
    os.chdir('../..')
    if os.path.exists('./4desk_portable.exe'):
        os.replace('./target/release/4desk-portable-packer.exe',
                   './4desk_portable.exe')
    else:
        os.rename('./target/release/4desk-portable-packer.exe',
                  './4desk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/4desk_portable.exe')
    os.rename('./4desk_portable.exe', f'./4desk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/4desk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/4desk.exe')
        system2('mv target/release/4desk.exe target/release/4desk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\4desk.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/4desk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/4desk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/4desk-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/4desk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv 4desk-%s-0-x86_64.pkg.tar.zst 4desk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./4desk.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/4desk-%s-0.x86_64.rpm ./4desk-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall 4desk.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/4desk-%s-0.x86_64.rpm ./4desk-%s-suse.rpm' % (
                version, version))
        # yum localinstall 4desk.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/4desk*.deb ./flutter/4desk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/4desk.app/Contents/MacOS/4desk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/4desk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/4desk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/4desk.app/Contents/MacOS/4desk
    #rcodesign sign --p12-file ~/.p12/4desk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/4desk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/4desk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/4desk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/4desk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/4desk.app
    '''.format(pa))
                system2(
                    'create-dmg "4desk %s.dmg" "target/release/bundle/osx/4desk.app"' % version)
                os.rename('4desk %s.dmg' %
                          version, '4desk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/4desk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./4desk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./4desk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple 4desk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/4desk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/4desk*.deb ./4desk.deb')
                system2('dpkg-deb -R 4desk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/4desk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/4desk-link.desktop tmpdeb/usr/share/applications/4desk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/4desk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/4desk/')
                os.system('mkdir -p tmpdeb/etc/X11/4desk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/4desk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/4desk.debian tmpdeb/etc/pam.d/4desk')
                system2('strip tmpdeb/usr/bin/4desk')
                system2('mkdir -p tmpdeb/usr/share/4desk')
                system2('mv tmpdeb/usr/bin/4desk tmpdeb/usr/share/4desk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/4desk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb 4desk.deb; /bin/rm -rf tmpdeb/')
                os.rename('4desk.deb', '4desk-%s.deb' % version)
