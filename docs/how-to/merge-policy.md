...
| Feature / fix / docs PR | **Squash** | One issue = one PR = one commit on `4desk/main`; readable history, trivial reverts; strict-mode refresh commits disappear. |
...
When a stacked PR's base merges, GitHub retargets it; if a stacked PR still auto-closes, recreate it against `4desk/main` and comment why on the closed one.
